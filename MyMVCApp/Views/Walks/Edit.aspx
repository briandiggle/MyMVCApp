        ...
        $('#getimages').click(function () {
            ...
            $.get('/Walks/CheckImages', { imagepath: $("#images_path").val() }, function (oResults) {
                ...
                for (iCount = 1; iCount <= oResults.imagesfound; iCount = iCount + 1) {
                    $("#walkimages").append('<br/><b>New Image ' + iCount + '</b><br/><input type="text" name="imagecaption' + iCount + '" size="100" />');
        ...
        <fieldset>
           ...
           <div class="editor-label">
            ...
            <% For iHillAscentsCounter = 1 To Model.HillAscents.Count%>
             <div class="editor-field" id="Div1">
                <%: Html.TextBox("VisitedSummit" + iHillAscentsCounter.ToString, WalkingMVC.WalkingMVC.Helpers.WalkingStick.FormatHillSummaryAsLine(Model.HillAscents(iHillAscentsCounter - 1).Hill) + " ", New With {.size = 80})%> <%: Html.Hidden("VisitedSummit" + iHillAscentsCounter.ToString + "HillID", Model.HillAscents(iHillAscentsCounter - 1).Hillnumber.ToString)%>
            </div>
            <% Next%>      
            ...
            <% Next%>
            <%  Dim oWalkMarkersAlreadyCreated As List(Of WalkingMVC.WalkingMVC.Marker) = ViewData("WalkMarkersAlreadyCreated")
            ...
            <% For Each oWalkMarker As WalkingMVC.WalkingMVC.Marker In oWalkMarkersAlreadyCreated%>
            <tr>
          ...
          &nbsp;<div class="editor-field" id="WalkMarkers">
           ...
           <% 
            ...
            <% Else%>
             <input type="checkbox" id="existingimageismarker<%=iExistingImageCount.ToString %>" name="existingimageismarker<%=iExistingImageCount.ToString %>" /></span>

            <% End If %>
 
            <% If ViewData("AT_WORK") = "True" Then%>       
                   &nbsp;<%= WalkingMVC.WalkingMVC.Helpers.WalkingStick.ExtractFileNameFromPath(oWalkImage.Walk_AssociatedFile_Name) %><br />
            <% Else %>
           ...
           <% 
               Dim IQAuxilliaryFiles As IEnumerable(Of WalkingMVC.WalkingMVC.Walk_AssociatedFile) = ViewData("Auxilliary_Files")
